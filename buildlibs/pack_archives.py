...
def compile (src,dest):
    ...
    if dest.endswith('.so'):
        os.system(sys.executable+' -m nuitka --module app/cache/archives/code/'+src)
        os.system ('mv -i *.so '+' app/cache/archives/data/'+dest+' && rm -r *.build *.pyi')
    else:
        os.system(sys.executable + ' -m nuitka --follow-import-to=PyQt5 -o app/cache/archives/data/'+dest+' app/cache/archives/code/' + src)
...
def unpack (name):
    ...
    if os.path.isfile("app/cache/archives/control/compile"): shutil.copyfile("app/cache/archives/control/compile",
                                                                              "stor/app/packages/" + name + ".compile")
    if os.path.isfile("app/cache/archives/control/list"): shutil.copyfile("app/cache/archives/control/list",
